...
class EncoderDecorder(nn.Module):
    def __init__(self, level=5, norm='bn'):
        ...
        # self.encoder_res6 = Res3DBlock(128, 128, norm=norm)
    ...
    def forward(self, x, verbose=False):
        ...
        # print(f"skip_x5.shape = {skip_x5.shape}")

        x = self.mid_res(x)
        # _print(x, msg='after mid_res')
        # print(f"skip_x5.shape = {skip_x5.shape}")

        # print(f"skip_x5.shape = {x.shape}")
        # if self.level >= 6: x = self.encoder_pool6(x)
        # _print(x, msg='after encode 6')
        # print(f"skip_x6.shape = {x.shape}")
