...
def rotate_image(image, camera=None, landmarks=None, normals_image=None, preserve_range=False):
    # 1. Rotate the main image
    # print('rotating', image.dtype)
    image_rot = rotate(image, 90, resize=True, preserve_range=preserve_range)
    # print('rotated', image_rot.dtype)

    # 2. Rotate the normals image if provided
    normals_rot = None
    if normals_image is not None:
        normals_rot = rotate(normals_image, 90, resize=True)

    # 3. Prepare placeholders for the outputs
    camera_rot = None
    landmarks_rot = None

    # 4. If camera intrinsics are given, update them
    if camera is not None:
        # Build the rotation/translation matrix for intrinsics
        Rt = np.array([
            [ 0,  1,               0            ],
        ...
        # Copy camera dict so we don’t overwrite the original
        camera_rot = dict(camera)
        camera_rot['intrinsics'] = Rt.dot(camera['intrinsics'])
        # Swap width/height
        camera_rot['image_size'] = camera['image_size'][::-1]

        # If landmarks exist, rotate them using the same Rt
        if landmarks is not None:
            # Append a 1 to each (x,y) so we can do a homogeneous transform
            pts_h = np.concatenate((landmarks, np.ones((landmarks.shape[0], 1))), axis=1)
            pts_rot_h = Rt.dot(pts_h.T).T
            landmarks_rot = pts_rot_h[:, :2]

    # 5. If no camera was given but landmarks were given, rotate landmarks in image space
    elif landmarks is not None:
        # Build the rotation/translation matrix for image coordinates
        Rt_img = np.array([
            [ 0,  1,               0            ],
            [-1,  0,  image_rot.shape[1]      ],
            [ 0,  0,               1            ]
        ])
        pts_h = np.concatenate((landmarks, np.ones((landmarks.shape[0], 1))), axis=1)
        pts_rot_h = Rt_img.dot(pts_h.T).T
        landmarks_rot = pts_rot_h[:, :2]
    ...
    result = {
        'image': image_rot,
        'camera': camera_rot,
        'landmarks': landmarks_rot,
        'normals_image': normals_rot
    }
    return result


# def rotate_image(image, camera=None, landmarks=None, normals_image=None):
#     image = rotate(image, 90, resize=True)

#     if normals_image is not None:
#         normals_image = rotate(normals_image, 90, resize=True)
...
#         if landmarks is not None:
#             # print(landmarks.shape, np.ones((landmarks.shape[0],1)).shape)
#             landmarks = np.concatenate((landmarks, np.ones((landmarks.shape[0],1))), axis=1)
#             landmarks = Rt.dot(landmarks.T).T[:,:2]
#             return image, camera, landmarks, normals_image

#         return image, camera, normals_image
#     else:
#         if landmarks is not None:
#             landmarks = np.concatenate((landmarks, np.ones((landmarks.shape[0],1))), axis=1)
...
#                 [ 0,    0,             1           ]
#             ]).dot(landmarks.T).T[:,:2]
#             return image, landmarks, normals_image
#         return image, normals_image
