...
def add_labels_to_images(images, labels, font_scale=0.6, thickness=1, font=cv2.FONT_HERSHEY_SIMPLEX):
    labeled_images = []
    for img, label in zip(images, labels):
        img_copy = img.copy()
        if img_copy.dtype != np.uint8:
            img_copy = (255 * np.clip(img_copy, 0, 1)).astype(np.uint8)  # Normalize if needed

        # Add a black strip at the top
        label_height = 20
        label_strip = np.zeros((label_height, img_copy.shape[1], 3), dtype=np.uint8)
        cv2.putText(label_strip, label, (5, label_height - 5), font, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)
        labeled_img = np.vstack((label_strip, img_copy))
...
def print_memory(device, prefix=''):
    ...
    proc = psutil.Process(os.getpid())         # this Python process
    rss_mb = proc.memory_info().rss / 1024**2  # resident‑set size → MB
    sys_used = psutil.virtual_memory()         # whole‑system RAM snapshot

    print(f"Process RAM: {rss_mb:.2f} MB")
    print(f"System RAM:  {sys_used.used/1024**2:.0f} MB "
        f"of {sys_used.total/1024**2:.0f} MB "
        f"({sys_used.percent} % used)")
...
def to_numpy(input):
    if torch.is_tensor(input):
        t = input.detach().cpu()
        # NumPy does not support bfloat16; ensure float-compatible dtype
...
def keep_vertices_and_update_faces(faces, vertices_to_keep):
    ...
    # elif not isinstance(vertices_to_keep, np.ndarray):
        # vertices_to_keep = torch.tensor(vertices_to_keep, dtype=torch.long)

    # print(type(vertices_to_keep))
