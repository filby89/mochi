...
class MeshHelper:
    ...
    def render_normals_2(self, vertices, camera_intrinsics, camera_extrinsics, radial_distortions=None, depth_eps=3e-3, eps=1e-7, depth_rendering_height=200, depth_rendering_width=200):
        '''
        Compute the vertex visibility with a thresholded depth rendering check. 
        ...
        # WARN: we use the normals w.r.t. the image 
        # vertex_normals = self.vertex_normals(vertices_image_rotated_3d.transpose(1, 2)) # (batch_size*num_views, num_vertices, 3)
        vertex_normals = self.vertex_normals(vertices_image_rotated_3d.transpose(1, 2)) # (batch_size*num_views, num_vertices, 3)
        vertex_normals[..., 1] = -vertex_normals[..., 1]
        vertex_normals[..., -1] = -vertex_normals[..., -1]


        # vertex_normals = torch.bmm(vertex_normals.repeat(batch_size*num_views, 1, 1), camera_extrinsics[:,:3,:3]) # (batch_size*num_views, num_vertices, 3)

        face_vertex_normals = self.face_vertices(vertex_normals, self.faces.unsqueeze(0).repeat(batch_size*num_views,1,1).to(device)) # (batch_size*num_views, num_faces, 3)
    ...
    def render_normals_3(self, vertices, camera_intrinsics, camera_extrinsics, radial_distortions=None, depth_eps=3e-3, eps=1e-7, depth_rendering_height=200, depth_rendering_width=200,
                         return_depth=False):
        device = vertices.device
        batch_size, num_vertices, _ = vertices.shape
        num_faces, _ = self.faces.shape
        ...
        v_cam      = vertices_image_rotated_3d.transpose(1,2)          # B*V × Nv × 3
        ...
        normal_images = rendered[...,:3]         # screen-space colours
        depth_images  = rendered[...,3:] if return_depth else None
...
#         extr_ref=None):         # (B, 1, 3, 4)  –  world→refCam, or None
#     """
...
#     pix   = torch.stack((uu, vv), dim=-1).float()       # (W,H,2)
#     pix   = pix.reshape(1,1,H,W,2).expand(B,V,-1,-1,-1)

#     # -------------------------------------------------
#     # 2. intrinsics
#     # -------------------------------------------------
#     fx = K[..., 0, 0].view(B, V, 1, 1, 1)
#     fy = K[..., 1, 1].view(B, V, 1, 1, 1)
#     cx = K[..., 0, 2].view(B, V, 1, 1, 1)
#     cy = K[..., 1, 2].view(B, V, 1, 1, 1)
...
#     pts_cam_flat = pts_cam.view(B*V, -1, 3)            # (B*V, HW, 3)
...
#     # -------------------------------------------------
#     # 4. world → refCam   (R_ref  x  + t_ref)
#     # -------------------------------------------------
#     Rr = extr_ref[..., :3]                              # (B,1,3,3)
#     tr = extr_ref[..., 3:].unsqueeze(-2)                # (B,1,1,3)

#     pts_ref = (pts_world - tr) @ Rr.transpose(-1, -2)
#     return pts_ref
