...
class FLAME(nn.Module):
    ...
    """
    def __init__(self, flame_model_path='assets/FLAME2023/flame2023_no_jaw.pkl', 
                 flame_lmk_embedding_path='assets/landmark_embedding.npy', n_shape=300, n_exp=100, no_jaw=False): #, to_mm=True):
        ...
        # if to_mm:
        #     self.v_template *= 1000.
        #     self.shapedirs *= 1000.
        #     self.posedirs *= 1000.
        #     print(self.v_template, self.shapedirs, self.posedirs)
            # self.lbs_weights *= 1000.
    ...
    def select_mediapipe(self, vertices):
    ...
    def forward(self, param_dictionary, zero_expression=False, zero_shape=False, zero_pose=False):
        ...
        vertex_offsets = param_dictionary.get('vertex_offsets', None)  # optional per-vertex offsets (meters)
        # translation_params = param_dictionary.get('translation_params', None)
        
        if jaw_params is None:
            jaw_params = torch.zeros(shape_params.shape[0], 3, dtype=self.dtype).to(shape_params.device)
        ...
        # if translation_params is not None:
            # vertices = vertices + translation_params.unsqueeze(1)
